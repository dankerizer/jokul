...
namespace Dankerizer\Jokul\Commons;
...
class JokulRequest{
    ...
    public  function post($config = array(),$params = array()){
        $header = array();
        $data = array(
            "order" => array(
        ...
        $getUrl = self::getBaseUrl($config['environment']);
        ...
        $signature =    self::generateSignature($header, $targetPath, json_encode($data), $config['shared_key']);
    ...
    {
        return $state ? self::PRODUCTION_BASE_URL : self::SANDBOX_BASE_URL;
    }
}
